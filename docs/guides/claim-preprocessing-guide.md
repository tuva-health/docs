...
#### Mapping
When mapping a raw data source to the input layer, we map the `member_id` from the raw data to both `patient_id` and `member_id`
in the input layer. Later on, Tuva EMPI software will determine if people with different `patient_id`s are the same person and assign the same `patient_id` to those people.
